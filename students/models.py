...
class ITEducator(models.Model):
    ...
    sertificate_front = models.FileField(upload_to='malaka_ser_front/', null=True, blank=True)
    sertificate_back = models.FileField(upload_to='malaka_ser_back/', null=True, blank=True)
    seria = models.CharField(max_length=3, default='MK')

    def __str__(self):
        return self.ism

    def generate_sertificate_id(self):
        last_student = ITEducator.objects.order_by('-sertificate_id').first()
        if last_student:
    ...
    def generate_sertificate_id_numeric(self):
        last_student = ITEducator.objects.order_by('-sertificate_id_numeric').first()
        if last_student:
    ...
    def generate_certificate(self):
        ...
        qr_code = os.path.join(settings.MEDIA_ROOT, f'malaka_qrcode/qr_code-{self.sertificate_id}.png')
        qr_code_img_back = os.path.join(settings.MEDIA_ROOT, f'malaka_qrcode/qr_code-{self.sertificate_id}.png')
        berilgan = f"{self.berilgan_vaqt}"
        amal_qilish_mudati = f"{self.amal_qilish}"
        ...
        self.add_text(prs, 0, Inches(3.82), Inches(5.64), Inches(1), Inches(1), berilgan, 12, black_color, )
        self.add_text(prs, 0, Inches(5.04), Inches(5.64), Inches(1), Inches(1), amal_qilish_mudati, 12, black_color, )
        self.add_text(prs, 0, Inches(2.55), Inches(5.64), Inches(1), Inches(1), seria, 12, black_color)
        modul_1p = f"{self.modul_1p}%"
        modul_1_ball = f"{self.modul_1_ball}ball/ points"
        modul_2p = f"{self.modul_2p}%"
        modul_2_ball = f"{self.modul_2_ball}ball/ points"
        modul_3p = f"{self.modul_3p}%"
        modul_3_ball = f"{self.modul_3_ball}ball/ points"
        umumiy_p = f"{self.umumiy_p}%"
        umumiy_ball = f"{self.umumiy_ball}ball/ points"
        # Slayd 1
    ...
    def overlay_qr_code_back(self, background_path, qr_code_path, output_path, position, qr_size):
        ...
        draw.text((770, 1190), f"{self.modul_1_ball}ball/ points", fill=black_color, font=seria_font)
        draw.text((770, 1260), f"{self.modul_1p}%", fill=black_color, font=seria_font)
        draw.text((770, 1480), f"{self.modul_2_ball}ball/ points", fill=black_color, font=seria_font)
        draw.text((770, 1540), f"{self.modul_2p}%", fill=black_color, font=seria_font)
        draw.text((770, 1740), f"{self.modul_3_ball}ball/ points", fill=black_color, font=seria_font)
        draw.text((770, 1810), f"{self.modul_3p}%", fill=black_color, font=seria_font)
        draw.text((2000, 1230), f"{self.umumiy_ball}ball/ points", fill=black_color, font=seria_font)
        draw.text((2000, 1520), f"{self.umumiy_p}%", fill=black_color, font=seria_font)
    ...
    def save(self, *args, **kwargs):
        ...
        # bu yerga qaysi silkaga qr code yaratish kerak bo'lsa o'sha silka qo'yilsin
        qr_code_img = qrcode.make(f"http://astrumsertifikat.pythonanywhere.com/student/MK{self.sertificate_id}")
        ...
        if not self.sertificate_front:
            ...
            qr_code_image_path_front = os.path.join(settings.MEDIA_ROOT,
                                                    f'malaka_qrcode/qr_code-{self.sertificate_id}.png')
            output_image_path_front = os.path.join(settings.MEDIA_ROOT,
                                                   f'malaka_ser_front/certificate-{self.sertificate_id}.png')
        ...
        if not self.sertificate_back:
            ...
            output_image_path_back = os.path.join(settings.MEDIA_ROOT,
                                                  f'malaka_ser_back/certificate-{self.sertificate_id}.png')
        ...
        if not self.pptx_file:
            ...
            self.pptx_file.save(f'{self.seria}-{self.sertificate_id}-{self.familya}-{self.sharif}.pptx',
...
class InteriorDesign(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        qr_code_img = qrcode.make(f"http://astrumsertifikat.pythonanywhere.com/student/3D{self.sertificate_id}")
        canvas = Image.new('RGB', (380, 380), 'white')
        canvas.paste(qr_code_img)
        buffer = BytesIO()
        canvas.save(buffer, format='PNG')
...
class FullStack(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        qr_code_img = qrcode.make(f"http://astrumsertifikat.pythonanywhere.com/student/FS{self.sertificate_id}")
        canvas = Image.new('RGB', (380, 380), 'white')
        canvas.paste(qr_code_img)
        buffer = BytesIO()
        canvas.save(buffer, format='PNG')
...
class DataSciense(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        qr_code_img = qrcode.make(f"http://astrumsertifikat.pythonanywhere.com/student/DS{self.sertificate_id}")
        canvas = Image.new('RGB', (380, 380), 'white')
        canvas.paste(qr_code_img)
        buffer = BytesIO()
        canvas.save(buffer, format='PNG')
...
class SoftWare(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        qr_code_img = qrcode.make(f"http://astrumsertifikat.pythonanywhere.com/student/SE{self.sertificate_id}")
        canvas = Image.new('RGB', (380, 380), 'white')
        canvas.paste(qr_code_img)
        buffer = BytesIO()
        canvas.save(buffer, format='PNG')
