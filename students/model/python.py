...
class Python(models.Model):
    ...
    series = models.CharField(max_length=3, default='PT')
    ...
    pptx_file = models.FileField(upload_to='pptx_python/', blank=True)
    qr_code = models.ImageField(upload_to='python_qrcode/', blank=True)
    certificate_front = models.FileField(upload_to='python_ser_front/', blank=True)
    ...
    def generate_certificate_id():
        last_student = Python.objects.order_by('-certificate_id').first()
    ...
    def generate_certificate_id_numeric():
        last_student = Python.objects.order_by('-certificate_id_numeric').first()
        if last_student:
    ...
    def generate_certificate(self):
        pptx_template_path = os.path.join(settings.MEDIA_ROOT, 'template/python.pptx')
        ...
        qr_code = os.path.join(settings.MEDIA_ROOT, f'python_qrcode/qr_code-{self.certificate_id}.png')
        ...
        self.add_text(prs, 0, Inches(2.6), Inches(5.63), Inches(1), Inches(1), series, 11, black_color, )
    ...
    def save(self, *args, **kwargs):
        ...
        qr_code_img = qrcode.make(f"https://certificate.astrum.uz/student/PT{self.certificate_id}")
        ...
        if not self.certificate_front:
            background_image_path_front = os.path.join(settings.MEDIA_ROOT, 'template/python.png')
            qr_code_image_path_front = os.path.join(settings.MEDIA_ROOT,
                                                    f'python_qrcode/qr_code-{self.certificate_id}.png')
            output_image_path_front = os.path.join(settings.MEDIA_ROOT,
                                                   f'python_ser_front/certificate-{self.certificate_id}.png')
        ...
        super(Python, self).save(*args, **kwargs)
